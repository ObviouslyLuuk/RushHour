...
class Display {
      ...
      <div id="content_top_div" style="display: grid; grid-template-columns: auto 60%; align-items: center; column-gap:5px;">
        <div id="content_top_left_div" style="height:100%;display:grid;align-items:end;">
          ...
          <div id="content_top_left_bar_div" style="display:grid; grid-template-columns:auto auto auto auto; column-gap:5px;">
            ...
            <button id="undo_btn" class="btn">Undo</button>
          </div>
        </div>
        <div id="content_top_right_div">
          <div id="content_top_right_canvas_div" style="position: relative; margin-top:5px">
            ...
            <canvas id="map_canvas" style="width:100%; border: 2px solid grey; border-radius: 5px; image-rendering: auto;"></canvas>
          </div>
  ...
  updateStats(game) {
    ...
    document.getElementById("best_score").innerHTML = game.best_moves.length
    document.getElementById("improved_score").innerHTML = game.improved_moves.length
    document.getElementById("avg_score").innerHTML = game.avg_score
    document.getElementById("current_score").innerHTML = `Moves: ${game.world.moves.length}`
  }
  ...
  drawMap(grid, exit) {
    ...
    let ctx = this.buffer.map

    let grid_height = grid.length
    let grid_width = grid[0].length

    ctx.canvas.height = this.context.map.canvas.height
    ctx.canvas.width = ctx.canvas.height * grid_width/grid_height
    ...
    // Draw border
    // roundRect(ctx, 0, 0, ctx.canvas.width, ctx.canvas.height, 20)

    let ratio = ctx.canvas.height / grid_height
    for (let i in grid) {
      // Draw gridlines
      let y = i*ratio
      ctx.beginPath()
      ...
      ctx.stroke()
      for (let j in grid[i]) {
        if (i == 0) { // Only draw vertical gridlines once
          let x = j*ratio
          ctx.beginPath()
          ctx.moveTo(x, 0)
        ...
        if (grid[i][j] == "x") {
          ...
          // Clear wall cell from solid background
          // ctx.clearRect(j*ratio, i*ratio, ratio, ratio)
        }
      }
    }
    ...
    let width = 15
    let height = 15
    let x = ctx.canvas.width -5
    let y = (exit+.5) * ctx.canvas.height/grid_height - height/2
    drawTriangle(x, y, width, height, "right", ctx, "white")
  }

  drawPieces(pieces, selected, grid_height, color=`rgb(${COOL_BLUE})`, space=10) {
    this.buffer.car.clearRect(0,0,2000,2000)
    this.context.map.clearRect(0,0,10000,10000)

    let ctx = this.buffer.car

    ctx.canvas.height = this.context.map.canvas.height
    ctx.canvas.width = this.context.map.canvas.width

    let ratio = ctx.canvas.height / grid_height

    for (let piece of Object.values(pieces)) {
      let use_color = color
      if (piece.id == "A") use_color = "red"

      let pos = {
        x: piece.x * ratio + space,
        y: piece.y * ratio + space,
      }
      if (selected && piece.id == selected.id) {
        let orie = piece.orientation
        let controller = document.value.controller
        let offset = controller.position[orie] - controller.start_pos[orie]
        offset = Math.max(offset, (piece.min-piece[orie])*ratio)
        offset = Math.min(offset, (piece.max-piece[orie])*ratio)
        pos[orie] += offset
      }
      let length = piece.length * ratio - space*2
      let broadness = ratio - space*2

      let width = broadness
      let height = length
      if (piece.orientation == "x") {
        width = length
        height = broadness
      }
      ...
      ctx.lineWidth = 3
      roundRect(ctx, pos.x, pos.y, width, height, 10, true, true)
    }
  }
  ...
  render() { 
    ...
    this.context.map.drawImage(this.buffer.car.canvas, 0, 0, this.buffer.car.canvas.width, this.buffer.car.canvas.height, 0, 0, this.context.map.canvas.width, this.context.map.canvas.height);   
...
function drawTriangle(x, y, width, height, direction, context, color="black", pointiness=.5) {
  ...
  let old_color = context.fillStyle
